...
| Init byte                                                         | Manufacturer ID                                                                                                                                                                                                                                                 | Device ID                                                                                                                                                                                                            | Model ID (assigned by the manufacturer)           | Command ID (assigned by the manufacturer)                                                                | Arguments (assigned by the manufacturer)                                    | Ending byte                                                        |
| ----------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------- | ------------------------------------------------------------------ |
| It is used to open every SysEx message. Its value is always `F0`. | Can take from 1 byte (`00`) to 3 bytes (`00 00 00`) on the message. Every single MIDI manufacturer is registered with the MIDI Association and has a unique ID. You can find them [here](https://www.midi.org/specifications-old/item/manufacturer-id-numbers). | This one is usually used for chain-connected MIDI devices, so that a SysEx message only targets one device of the chain rather than all of them. On MIDI over USB isn't normally used and it's default value is `01` | Should be unique for each device model they make. | It is usually used to specify what kind of information you are going to send/request to/from the device. | The actual body of the message you are sending. It has an unlimited lenght. | It is used to close every SysEx message. Its value is always `F7`. |
